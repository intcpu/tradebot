...
class RedisHash(object):
    ...
    def hdel(self, key, name):
        self.setHashKey(key)
        return self._db.hdel(self.hash_key, name)

    def hdelete(self, key):
        self.setHashKey(key)
        return self._db.delete(self.hash_key)
...
class redisClient(RedisStr, RedisHash, RedisQueue):
    def __init__(self, namespace='py', redis_host='127.0.0.1', redis_port=6379, select_db=0):
        super().__init__()
        self._db = redis.Redis(host=redis_host, port=redis_port, db=select_db, decode_responses=True)
...
if __name__ == '__main__':
    ...
    redis_db.hset('lever:risk:test', 1, 1)
