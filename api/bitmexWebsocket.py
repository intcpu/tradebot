...
class bitmexWebsocket(websocketClient):
    ...
    def on_packet(self, message):
        if 'error' in message:
            ...
            try:
                if action:
                    symbol = message['data'][0]['symbol'] if len(message['data']) > 0 and 'symbol' in message['data'][
                        0] else None
                ...
                self.on_action(action, table, symbol, message['data'])
            except:
    ...
    # 用户操作
    def on_action(self, action, table, symbol, data):
